...
Page({
  ...
  */
  onShow: function () {
    ...
    // 重新加载数据
    this.setData({ events: [], pageIndex: 0, hasMore: true });
    this.loadMore();
  },
  ...
  loadMore() {
    ...
    return fetch.reqMethod(`event/findIEventCom`, params)
      .then(res => {
        wx.hideLoading();
        if (res.data.result === "Sucess") {
          const totalCount = parseInt(res.data.data1.total);
          const hasMore = pageIndex * pageSize < totalCount;
          const events = this.data.events.concat(res.data.data1.eventMain);
          this.setData({ events, pageIndex, hasMore });
        } else {
          wx.showToast({
            title: '请求失败！',
            icon: 'success',
