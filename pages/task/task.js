...
Page({
  ...
  */
  onShow: function () {
    ...
    // 重新加载数据
    this.setData({ tasks: [], pageIndex: 0, hasMore: true });
    this.loadMore();
  },
  ...
  loadMore() {
    ...
    return fetch.reqMethod(`inspection/selInsTaskSearch`, params)
      .then(res => {
        wx.hideLoading();
        if (res.data.result === "Sucess") {
          const totalCount = parseInt(res.data.data1.total);
          const hasMore = pageIndex * pageSize < totalCount;
          const tasks = this.data.tasks.concat(res.data.data1.inss);
          this.setData({ tasks, pageIndex, hasMore });
        } else {
          wx.showToast({
            title: '请求失败！',
            icon: 'success',
